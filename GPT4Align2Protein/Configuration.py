...
class Config:
    ...
    def set_mode(self, mode):
        ...
        """
        job_config = {
            "train_path": f"{self.BASE_PATH + '1. Pretraining/'}datasets/{self.train_fname}",
            "val_path": f"{self.BASE_PATH + '1. Pretraining/'}datasets/{self.val_fname}",
        ...
        if mode == 'Pretraining':
            job_config.update({
                ...
                "epochs": 30,
                "learning_rate": 3e-4,
                "save_ckpt_path": f"{self.BASE_PATH + '1. Pretraining/'}model_weights/{self.train_ckpt_name}.pt",
                "desc_path": f"{self.BASE_PATH + '1. Pretraining/'}dataset_descriptors/{self.train_fname.split('.')[0][:-6]}.yaml",
                "load_ckpt_path": f"{self.BASE_PATH + '1. Pretraining/'}model_weights/{self.load_ckpt_name}"
            })
            ...
            job_config.update({
                ...
                "epochs": 10,
                "learning_rate": 3e-5,
                "save_ckpt_path": f"{self.BASE_PATH + '6. ActiveLearning/'}model_weights/{self.train_ckpt_name}.pt",
                "desc_path": f"{self.BASE_PATH + '6. ActiveLearning/'}dataset_descriptors/{self.dataset_desc_fname}.yaml",
                "load_ckpt_path": f"{self.BASE_PATH + '6. ActiveLearning/'}model_weights/{self.load_ckpt_name}"
