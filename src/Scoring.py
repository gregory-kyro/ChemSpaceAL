...
def plot_ligand_scores(config: dict, ligand_scores: dict):
    ...
    print(f'There are {zero_count} ligands with 0 connections ({np.round(zero_count / len(data)*100, 1)}%)')
    for i in range(int(np.max(data)) + 1):
        count = len([value for value in data if value > i])
        print(f'There are {count} ligands with scores greater than {i} ({np.round(count / len(data)*100, 1)}%)')
    print(f'The mean score for all ligands is {np.round(np.mean(data), 1)}')
    print(f'The lowest score for all ligands is {np.min(data)}')


def parse_and_prepare_diffdock_data(ligand_scores: dict, config: dict, lower_percentile=50, threshold=11, scored_db=None) -> pd.DataFrame:
    """
    ...
    all_ligands = {int(complex_name[7:]): score for complex_name, score in ligand_scores.items()}
